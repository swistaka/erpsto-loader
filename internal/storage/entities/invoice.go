...
func SaveInvoice(db *sql.DB, data []byte, entityHash string, usedProductIDs map[string]bool) error {
	...
	_, err = tx.Exec(`
		INSERT OR REPLACE INTO invoice (
			...
			warehouse_id, client_id, sum, sum_vat, comment, 
	...
	_, err = tx.Exec(`
		DELETE FROM invoice_position WHERE invoice_id = ?`,
		entity.ID,
	)
	if err != nil {
		return fmt.Errorf("failed to delete invoice position: %w", err)
	}
	...
	for _, pos := range entity.Positions {
		...
		_, err = tx.Exec(`
			INSERT OR REPLACE INTO invoice_position (
				id, invoice_id, product_id, amount, price, price_without_vat, 
	...
	// Сохраняем хэш
	if err := storage.SaveEntityHash(tx, "invoice", entity.ID, entityHash); err != nil {
		return fmt.Errorf("failed to save invoice hash: %w", err)
	}
