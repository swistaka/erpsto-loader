...
type Realization struct {
	...
	Positions  []RealizationPosition `json:"positions"`
}
...
func SaveRealization(db *sql.DB, data []byte, entityHash string, usedProductIDs map[string]bool) error {
	...
	_, err = tx.Exec(`
		INSERT OR REPLACE INTO realization (
			id, ext_index, doc_date, doc_number, status, sto_id, 
			client_id, contract_id, sum, sum_vat, comment, 
	...
	if err != nil {
		return fmt.Errorf("failed to save realization: %w", err)
	}

	// Удаляем старые строки ТЧ.Товары
	_, err = tx.Exec(`
		DELETE FROM realization_position WHERE realization_id = ?`,
		entity.ID,
	)
	if err != nil {
		return fmt.Errorf("failed to delete realization position: %w", err)
	}
	...
	for _, pos := range entity.Positions {
		...
		_, err = tx.Exec(`
			INSERT OR REPLACE INTO realization_position (
				id, realization_id, product_id, amount, price, price_without_vat, 
	...
	// Сохраняем хэш
	if err := storage.SaveEntityHash(tx, "realization", entity.ID, entityHash); err != nil {
		return fmt.Errorf("failed to save realization hash: %w", err)
	}
